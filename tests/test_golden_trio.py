...
def _long_setup_candles():
    """Noisy uptrend (RSI hovers ~55, not pinned at 100), then a sharp
    pullback that stamps a new Donchian low + drives RSI < 20, then a
    bullish reversal trigger with a long lower wick tagging the band."""
    candles = make_candles(300, start_price=1800.0, step=3.0, noise=3.0)
    price = candles[-1]["c"]
    for _ in range(11):
        c_next = price - 14.0
        candles.append({"o": price, "h": price + 0.3, "l": c_next - 0.3, "c": c_next, "v": None})
        price = c_next
    trigger_close = price + 55.0
...
def _short_setup_candles():
    candles = make_candles(300, start_price=2700.0, step=-3.0, noise=3.0)
    price = candles[-1]["c"]
    for _ in range(11):
        c_next = price + 14.0
